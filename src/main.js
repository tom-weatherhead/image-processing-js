...
/*
From https://github.com/yglukhov/bicubic-interpolation-image-processing/libimage.c :
...
/* function getBicubicWeight(x) {
	x = Math.abs(x);

	if (x < 1) {
		return (1.5 * x - 2.5) * x * x + 1;				// This will be positive.
	} else if (x < 2) {
		return ((-0.5 * x + 2.5) * x - 4) * x + 2;		// This will be zero (if x === 1) or negative.
	} else {
		return 0;
	}
} */
...
function resampleImageFromJpegFile(srcFilePath, dstFilePath, dstWidth, dstHeight, dstQuality, mode) {
	...
	const dstImage = resampleImageFromBuffer(srcImage, dstWidth, dstHeight, mode);
